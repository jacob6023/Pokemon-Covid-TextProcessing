...
def method_3(f):
    ...
    # Lists to store the attack, defense, and hit points for pokemons with levels above and below the threshold
    atk_above_threshold = []
    def_above_threshold = []
    hp_above_threshold = []
    atk_below_threshold = []
    ...
    with open(f, mode='r', newline='') as file:
        ...
        for row in csv_reader:
            ...
            # Check if atk, def, and hp are NaN
            if atk != 'NaN':
                atk = float(atk)
            else:
                atk = None
...
def method_5(f):
    ...
    print(f"Average hit point for Pokemons of stage {target_stage} = {average_hp}")
